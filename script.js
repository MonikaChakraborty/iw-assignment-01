...
function calculateBudget() {
  ...
  const totalExpenseValue = parseFloat(
    totalExpense.innerHTML.replace(/,/g, ""),
  );
...
function deleteEntry(event) {
  ...
  if (deleteItem.classList.contains("delete-item")) {
    const listItem = deleteItem.parentNode.parentNode.parentNode;
...
function addEntry() {
  ...
  const enteredExpense = parseFloat(value);

  
  if (type === "expense" && totalExpenseValue + enteredExpense > totalIncomeValue) {
    alert("Expense cannot exceed total income.");
    return;
  }
